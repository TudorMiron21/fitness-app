...
unique_equipment = df[['equipment_details', 'Equipment_x']].drop_duplicates()
unique_equipment = unique_equipment.dropna(subset=['Equipment_x'])
print(unique_equipment)

try:
    for index, equipment in unique_equipment.iterrows():
        # Check if the equipment already exists to avoid duplicates
        exists = session.query(equipment_table).filter_by(name=equipment['Equipment_x']).first() is not None
        if not exists:
            # Construct an insert statement for the equipment_table
            ins = equipment_table.insert().values(
                name=equipment['Equipment_x'], 
                equipment_details_url=equipment['equipment_details']
            )
