  ...
  const onDelete = async (exerciseId) => {
    ...
    try {
      const response = await axios.delete(
        `https://fit-stack.online/api/v1/adminCoachService/coach/deleteExercise/${exerciseId}`,
        {
          headers: {
    ...
    const fetchWorkouts = async () => {
      try {
        const response = await axios.get(
          "https://fit-stack.online/api/v1/adminCoachService/coach/getAllExercisesForCoach",
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("access_token")}`,
            },
